...
function apiQuoteDecision(payload){payload=payload||{};return quoteApi_('decision',function(){var repo=new QuoteRepository(),quote=repo.find(payload.quoteId,false);if(!quote)throw new Error('Quote not found.');var decision=String(payload.decision||'');if(JSK_QUOTE_SCHEMA.DECISION_VALUES.indexOf(decision)===-1)throw new Error('Select a valid client decision.');return repo.update(quote.quoteId,{clientDecision:decision,status:decision==='Approved'?'Approved':decision==='Rejected'?'Rejected':quote.status},payload.actor||'Quote Decision UI',quote.recordVersion);});}
